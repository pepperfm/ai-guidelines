...
- Если MCP недоступен или возвращает 500, то при наличии в проекте папки `.ai/nuxtui/`:
  - Использовать локальное зеркало Nuxt UI доков в `.ai/nuxtui/raw/docs/**`.
  - Использовать `.ai/nuxtui/llms.txt` как индекс для поиска нужного markdown-файла.
  - Для props/slots/examples открывать только 1–2 релевантных `.md`, не “сканировать всё”
