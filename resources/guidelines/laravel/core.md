...
- **Разрешено** запускать `bun`‑скрипты через Sail с хоста, **только** для фронтенд‑инструментов
  (lint/typecheck/build) и **только по явному запросу пользователя**.
- Используем форму: `./vendor/bin/sail bun run <script>`.
