...
Точные сигнатуры/поведение см. в skill `laravel-array-macros`.
...
### 8.1 `str()` и получение обычной PHP-строки

Разделяй два сценария:

1. `str()->uuid()` / `str()->uuid7()` / `str()->orderedUuid()`
2. `str('...')->...` fluent chain, которая возвращает `Illuminate\Support\Stringable`

Правила:
- если нужен **UUID как строка**, пиши `str()->uuid()->toString()`;
- если нужен **обычный string из `Stringable`**, пиши `->value()`;
- не используй `->toString()` как общий способ "достать строку" из fluent `Stringable`;
- `->toString()` допустим как осознанный UUID-oriented extraction, но для обычного fluent string в проекте стандарт — `->value()`.
...
$name = str($payloadName)->trim()->toString();
```
...
Интерполяция в строках **разрешена широко**. Агент не должен излишне "упрощать" такие строки через временные переменные, конкатенацию или вынос выражения наружу, если исходная интерполяция уже читается нормально.

Главный принцип:
- если выражение можно безопасно и читаемо интерполировать прямо в строку, так и делаем;
- **фигурные скобки используем только когда они действительно нужны**:
  - для сложного выражения;
  - для массивного доступа;
  - для вызова метода;
  - для null-safe chain;
  - для снятия синтаксической двусмысленности.

### 9.1 Без скобок, если можно
...
### 9.2 В скобках, если выражение сложное
...
Такие строки **не нужно** автоматически переписывать в:
...
### 9.3 Когда точно нужны фигурные скобки

- массивный доступ: `"{$payload['limit']}"`
- вызов метода: `"{$service->resolveUserName($user)}"`
- null-safe / длинная цепочка: `"{$post?->author?->profile?->display_name}"`
- если после переменной идут символы, из-за которых имя может читаться неоднозначно
...
### 9.4 Что считать нормальным стилем

- простая переменная -> без скобок;
- простое свойство объекта -> без скобок;
- всё остальное можно оставлять прямо в строке, но уже в `{...}`;
- не "нормализовать" сложную интерполяцию во временные переменные без реальной причины;
- не переписывать интерполяцию в конкатенацию без причины;
- не выносить выражение во временную переменную без причины.
...
- если один из импортов является **родительским классом** (`extends BaseClass`), ставь его **самым первым среди всех `use`-импортов файла**;
- после родительского класса идут остальные импорты в обычном порядке по правилам выше.
